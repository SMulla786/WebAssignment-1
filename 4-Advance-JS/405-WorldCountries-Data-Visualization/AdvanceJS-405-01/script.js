...
function init() {
  ...
  languagesButton.addEventListener(
    "click",
    setAnalyticsLanguageVsNumberOfCountries
  );
}
    ...
    html += `<div class="analytics-row">
        ...
        <div class="bar_container">
          <div class="bar" style="width: ${
            (countryVsPopulation[i].population / totalPopulation) * 100
          }%"></div>
        </div>
...
function setAnalyticsLanguageVsNumberOfCountries() {
  ...
  for (let language in languageVsNumberOfCountries) {
    html += `<div class="analytics-row">
        ...
        <div class="bar_container">
          <div class="bar" style="width: ${
            (languageVsNumberOfCountries[language] / countries_data.length) *
            100
          }%"></div>
        </div>
...
function sortedLanguageVsNumberOfCountriesData() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let j = 0; j < numberOfCountries.length; j++) {
      if (
        numberOfCountries[j] &&
        numberOfCountries[j] >= numberOfCountries[maxIndex]
      ) {
        maxIndex = j;
      }
    }
    newLanguagesVsNumberOfCountries[languages[maxIndex]] =
      numberOfCountries[maxIndex];
